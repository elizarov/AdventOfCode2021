fun main() {
    ...
    val ux = a.flatMap { listOf(it.x1, it.x2 + 1) }.distinct().sorted()
    val uy = a.flatMap { listOf(it.y1, it.y2 + 1) }.distinct().sorted()
    val uz = a.flatMap { listOf(it.z1, it.z2 + 1) }.distinct().sorted()
    val g = Array(ux.size) { Array(uy.size) { IntArray(uz.size) } }
    ...
    for (c in a) {
        for (x in mx[c.x1]!! until mx[c.x2 + 1]!!)
            for (y in my[c.y1]!! until my[c.y2 + 1]!!)
                for (z in mz[c.z1]!!until mz[c.z2 + 1]!!) {
                    g[x][y][z] = c.m
                }
    }
    var ans = 0L
    for (x in 0 until ux.size - 1) for (y in 0 until uy.size - 1) for (z in 0 until uz.size - 1) {
        if (g[x][y][z] == 1)
            ans += (ux[x + 1] - ux[x]).toLong() * (uy[y + 1] - uy[y]) * (uz[z + 1] - uz[z])
    }
    println(ans)
}
